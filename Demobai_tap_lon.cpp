...
void xuatds(Sinhvien *s,int n){
	...
	printf("%-10s %-23s %-5s %-5s %-5s %-5s %-8s %-10s\n", 
	"MSSV", "Ho va ten",  "Gioi Tinh", "Ngay ","Thang","Nam","DTB","Xep loai");
	for(int i=0;i<n;i++){
		printf("%-10s %-23s %-10s %-5d %-5d %-4d %-9.1f ",(s+i)->mssv,(s+i)->hoten,(s+i)->sex,(s+i)->day,
...
int main(){
	...
	do{
	 ...
	 if(kbhit()){
			...
			printf("\n\tBan co muon tiep tuc chuong trinh khong? (an 1 de tiep tuc): ");
	       scanf("%d",&tieptuc);
